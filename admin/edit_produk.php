...
if (isset($_POST['edit_produk'])) {
    ...
    if (!empty($_FILES['gambar']['name'])) {
        ...
        $update = $conn->prepare("UPDATE produk SET nama_produk=?, harga=?, stok=?, kota_asal=?, lat=?, lng=?, deskripsi=?, gambar=? WHERE id=?");
        $update->execute([$nama, $harga, $stok, $kota_asal, $lat, $lng, $deskripsi, $gambar_baru, $id_edit]);
    } else {
        $update = $conn->prepare("UPDATE produk SET nama_produk=?, harga=?, stok=?, kota_asal=?, lat=?, lng=?, deskripsi=? WHERE id=?");
        $update->execute([$nama, $harga, $stok, $kota_asal, $lat, $lng, $deskripsi, $id_edit]);
    }
...
<head>
    <meta charset="UTF-8">
    <title>Edit Produk - XrivaStore</title>
                    ...
                    <div class="col-md-8">
                        ...
                        <div class="row mb-3">
                            ...
                            <div class="col-md-4">
                                ...
                                <input type="text" id="kota_input" name="kota_asal" class="form-control" value="<?= htmlspecialchars($p['kota_asal']) ?>" placeholder="Cth: Jatimulya">
                            </div>
                        ...
                        <div class="row mb-3 bg-light p-2 rounded">
                            <div class="col-md-6">
                                ...
                                <input type="text" name="lat" id="lat_input" class="form-control form-control-sm" value="<?= $p['lat'] ?>" readonly>
                            </div>
                            <div class="col-md-6">
                                <label class="small text-muted">Longitude</label>
                                <input type="text" name="lng" id="lng_input" class="form-control form-control-sm" value="<?= $p['lng'] ?>" readonly>
                            </div>
                        </div>

                        <div class="mb-3">
                            <label class="form-label fw-bold">Deskripsi</label>
                            <textarea name="deskripsi" class="form-control" rows="5"><?= htmlspecialchars($p['deskripsi']) ?></textarea>
                        </div>
                    </div>

                    <div class="col-md-4 border-start ps-4 text-center">
                        <label class="form-label fw-bold d-block text-start">Gambar</label>
                        <img src="../frontend/images/produk/<?= $p['gambar'] ?>" class="img-fluid rounded mb-3 border" style="max-height: 200px; width: 100%; object-fit: cover;">
                        <input type="file" name="gambar" class="form-control mb-4">
                        <button type="submit" name="edit_produk" class="btn btn-sage w-100 py-3 rounded-pill fw-bold">Simpan Perubahan</button>
                    </div>
                </div>
            </form>
        </div>
...
function cariLokasi() {
    ...
    if(!kota) return alert("Ketik nama lokasi!");
    ...
    .then(data => {
        if(data.length > 0) {
            ...
            alert("Lokasi " + kota + " ditemukan!");
        } else {
            alert("Lokasi tidak ketemu, coba nama yang lebih umum.");
        }
    });
}
</script>
